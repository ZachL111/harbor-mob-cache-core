...
This note keeps the mobile workflows assumptions visible beside the checks.
...
The widest spread is between `form pressure` and `sync drift`, so those are the first two cases I would preserve during a refactor.
